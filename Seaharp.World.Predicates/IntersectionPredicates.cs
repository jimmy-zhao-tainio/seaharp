...
public static class IntersectionPredicates
{
    ...
    public static bool Intersects(Seaharp.Geometry.Tetrahedron a, Seaharp.Geometry.Tetrahedron b)
    {
        ...
        // Note: this first step does not yet detect face-face crossings with
        // no vertices strictly inside. Those will be handled in a follow-up.
        return false;
    }
    ...
    public static bool Intersects(Seaharp.World.Shape first, Seaharp.World.Shape second)
    {
        ...
        for (int i = 0; i < a.Count; i++)
        {
            ...
            {
                if (Intersects(a[i], b[j])) return true;
            }
        }
        return false;
    }
    ...
    public static bool HasSelfIntersections(Seaharp.World.Shape shape)
    {
        ...
        for (int i = 0; i < tets.Count; i++)
        {
            ...
            {
                if (Intersects(tets[i], tets[j])) return true;
            }
        }
        return false;
    }

    private static bool AabbOverlap(in Seaharp.Geometry.Tetrahedron a, in Seaharp.Geometry.Tetrahedron b)
    {
        (long minX, long minY, long minZ, long maxX, long maxY, long maxZ) Box(in Seaharp.Geometry.Tetrahedron t)
        {
    ...
    private static bool PointInsideTetStrict(in Seaharp.Geometry.Tetrahedron t, in Point p)
    {
        // Use planes derived from outward faces; inside = negative side for all
        static bool Neg(in Seaharp.Geometry.Tetrahedron.Triangle tri, in Point q)
