...
public class SurfaceExtractionConsistencyTests
{
    ...
    private static void AssertConsistent(Shape shape)
    {
        ...
        var viaKey = new HashSet<Seaharp.ClosedSurface.TriangleKey>();
        foreach (var t in shape.ToClosedSurface().Triangles) viaKey.Add(Seaharp.ClosedSurface.TriangleKey.FromTriangle(t));
        ...
        var counts = new Dictionary<Seaharp.ClosedSurface.TriangleKey, int>(all.Count);
        for (int i = 0; i < all.Count; i++)
        {
            var keyI = Seaharp.ClosedSurface.TriangleKey.FromTriangle(all[i]);
            int c = 0;
            for (int j = 0; j < all.Count; j++)
            {
                if (i == j) continue;
                var keyJ = Seaharp.ClosedSurface.TriangleKey.FromTriangle(all[j]);
        ...
        var viaScan = new HashSet<Seaharp.ClosedSurface.TriangleKey>();
